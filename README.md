...
- `profiles` の項目は `icon: 'orcid' | 'github' | 'scholar'` を付けると、横長画面ではアイコン＋名前、スマホではアイコンだけの
  丸いボタンになる（アイコンの図形は `app.js` の `ICONS`）。`icon` が無い項目・知らない名前の項目は、これまでどおり `label` の文字が出る。
- CVのボタンの文言は `labels.cv`（`CV` / `履歴書`）。後ろの「 (PDF)」は `app.js` が足すので、スマホ表示のときだけ省ける。
...
- ボタン列では、メールと ORCID / GitHub / Google Scholar はアイコンだけの丸いボタンになり、CVは「 (PDF)」を省きます。
  メールを押したときの「コピー済み」は文字が出せないので、封筒のアイコンがチェックに変わります。

## サイドバーの高さ（画面が低いとき）

- 横長画面でサイドバーの中身が画面より高いときは、中でスクロールさせません（ボタン等は何も隠れない）。サイドバーは中身の高さの
  ままページと一緒に上へ動き、下端が画面下端に来たところで止まります。高さを測るのは `app.js` の `fitSidebar()`、
  見た目は `style.css` の `.sidebar.tall`。中身が画面に収まるときは従来どおり画面の高さに固定されます。
