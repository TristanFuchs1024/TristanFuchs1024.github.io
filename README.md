...
- `profiles` の項目は `icon: 'orcid' | 'github' | 'scholar'` を付けると、横長画面ではアイコン＋名前、スマホでは名前だけの
  ボタンになる（アイコンの図形は `app.js` の `ICONS`）。`icon` が無い項目・知らない名前の項目は、これまでどおり `label` の文字が出る。
...
- ボタン列は横にスクロールする文字のボタンのまま（アカウントのアイコンはスマホでは出さない）。
